...
public class Application1 {
    public static void main(String[] args) {
        ...
        *  - 즉, 같은 데이터 끼리만 연산을 수행할 수 있다.
         */

        /*필기.
        * 형변환의 종류와 규칙
        * 자동 형변환(묵시적 형변환) : 컴파일러가 자동으로 수행해주는 타입 변환
        *  1-1. 작은 자료형에서 큰 자로형으로는 자동 형변환 된다.
        *  1-2. 정수는 실수로 자동 형변환 된다.
        *  1-3. 문자형은 int형으로 자동 형변환 된다.
        *  1-4. 논리형은 형변환 규칙에서 제외된다.
        * 2. 강제형변환(명시적 형변환) : 형변환 (cas) 연산자를 이욯나 강제적으로 수행하는 형변환
        *  2-1. 자동형변환이 적용되지 않은 경우 강제 형변환이 필요하다.
        *
         */

        /*필기. 점점 더 큰 자료형으로 데이터를 옮겨도 문제 없이 자동 형 변환 처리가 된다.
         */
        byte bnum = 1;
        short snum = bnum;
        int inum = snum;
...
//        int result = num1 + num2;   //자동으로 큰 쪽 자료형인 long으로 변경 후 계산하기 때문에 int형 변수에 담을 수 없다.
        ...
        /*필기.
        * 정수를 실수로 변경할 때 소수점 자리수가 실수형태로 표현이 가능하다.
