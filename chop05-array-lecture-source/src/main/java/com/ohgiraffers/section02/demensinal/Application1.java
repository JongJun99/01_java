...
//        iarr1 = new int[][];  크기를 지정하지 않아 에러 발생
        iarr2 = new int[3][];
//        iarr3 = new int [][3];    주소를 묶어서 관리할 배열의 크기를 지정하지 않으면 에러 발생
        iarr3 = new int[3][];
        ...
        * 가변배열 : 서로 길이가 다른 배열 여러 개를 하나롸 묶어 관리하는 2차원 배열 */
        ...
        for(int i = 0; i < iarr7.length; i++) {
            for(int j = 0; j < iarr7[i].length; i++){
        ...
        for(int i = 0; i < iarr8.length; i++){
            for(int j = 0; j <iarr8.length; i++) {
                System.out.println("iarr8[" + i + "][" + j + "] :" + iarr8[i][j]);
            }
        }
        /*생선한 여러 개의 1차원 배열에 접근 사용*/
        iarr1[0][0] = 1;
        iarr1[0][1] = 2;
        iarr1[0][2] = 3;
