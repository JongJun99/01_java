...
public class FinalFieldTest {
    ...
    * 3. 전역변수 : 인스턴스 생성 후 초기화 이후에는 값 변경 불가
...
//    private final int nocStaticNum;     0으로 초기화되어 이후 변경 불가능하기 때문에 에러남
...
//    private final int STATIC_NUM;  //0으로 초기화 되어 이후 변경 불가능
    private static final int STATIC_NUM = 1;

    /*생성자를 이용한 초기화 불가능하다.
    * 생성자는 인스턴스가 생성되는 시점에 호출이 되기 때문에 그 전에는 초기화가 일어나지 못한다.
    * 하지만 static은 프로그램이 start 될 때 할당하기 때문에 초기화 되지 않은 상태로 선언된 것과 동일하여
