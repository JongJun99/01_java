...
public class User {
    ...
    private String pwd;
    private String name;
    private java.util.Date enrollDate;  //가입한 날짜

    /*생성자의 사용 목적
     * 1. 인스턴스 생성 시점에 수행할 명령이 있는 경우 사용한다.
     * 2. 매개변수 있는 생성자의 경우 매개변수로 전달받은 값으로 필드를 초기화하며, 인ㅇ스턴스를 생성할 목적으로 주로 사용
     ...
     * 2. 매개변수가 있는 생성자
     *  기본 생성자는 자바 Compiler가 자동으로 추가해주는 구문이지만 명시적으로 작성할 수 도 있다.
     *  매개변수는 있는 생성자가 한 개라도 존재하는 경우 기본생성자를 자동으로 추가해주지 않기 때문에
    ...
    /*동일한 이름의 생성자 혹은 메소드를 한 클래스 안에서 작성하는 것은 불가능하다.(Compiler Rrror) */
//    public User() {}
    public User(String id, String pwd, String name) {
        ...
        System.out.println("User 클래스와 id, pwd, name을 초기화하는 생성자 호출함");
    }
