...
public class Application {
    public static void main(String[] args) {
        ...
        /*추상화란 ?
        * 공통된 부분을 추출하고 공통되지 않은 부부능ㄹ 제거한다는 의미, 추상화의 목적은 유연성을 확보하기 위함이다.
        * 유연성 확보는 여러 곳에 적용될 수 있는 유연한 객체를 의미하며, 즉 재사용성이 높아질 수 있게 한다.
        * 객체의 재사용성이 증가하면 중복 작성되는 코드를 줄일 수 있으며, 오류 발생가능성을 감소시키고 유지보수성을 증가시킨다.*/

        /*시스템 요구사항
        * 1. 운전자는 시동걸기, 엑셀 밟기, 브레이크 밝기, 시동 끄기를 할 수 있다.
        * 2. 자동차는 시동걸기, 앞으로가기, 멈추기, 시동끄기를 할 수 있다.
        * 3. 자동차는 처음에 멈춘 상태로 대기하고 있는다.(시동이 꺼져있다.)
        * 4. 운전자는 먼저 자동차에 시동을 건다. 이미 걸려있는 경우 다시 시동을 걸 수 없다.
        * 5. 운전자가 엑셀을 밟으면 시동이 걸린 상태일 경우 자동차는 시속10km/h 증가하며 앞으로 나간다.
        ...
        *   1)carDriver 클래스
        *       -속성 : 자동차(Car 클래스)
        *       -행위 : 시동걸기, 엑셀밟아라, 브레이크 밟아라, 시동끄기
        *   2)car 클래스
        *       -속성 : 시동 상태, 현재시속,
        *       -행위 : 시동걸기, 앞으로가기, 멈추기, 시동끄기*/
