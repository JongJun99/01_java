...
public class KindsOfVariable {  //클래스영역의 시작
    ...
    /*non-static field 인스턴스 변수라고 한다. ( 인스턴스 생성 시점에 사용 가능한 변수라는 의미*/
    private int globalNum;

    /*static field를 정적필드(클래스변수)라고 한다. 클래스 여영ㄱ에 생성되는 변수라는 의미이다.*/
    private static int staticNmu;

    public void testMethod(int num) {   //메소드영역의 시작

        /*메소드의 괄호 안에 선언하는 변수를 매개변수라고한다.
...
//        System.out.println(localNum);     //지역변수는 해당 지역(블럭 내에서만 사용 가능ㅎ다.)
